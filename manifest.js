...
async function getVisibleManifestForUser(user) {
  ...
  return manifest
    .filter((section) => section.visible && batchAllowed(section, user))
    .map((section) => ({
      id: section.id,
      title: section.title,
      chapters: section.chapters
        .filter((ch) => ch.visible && batchAllowed(ch, user))
...
async function canServeChapter(user, sectionId, chapterId) {
  ...
  if (!section || !section.visible || !batchAllowed(section, user)) {
    return { ok: false, reason: 'not-found' };
  }
  const chapter = section.chapters.find((c) => c.id === chapterId);
  if (!chapter || !chapter.visible || !batchAllowed(chapter, user)) {
    return { ok: false, reason: 'not-found' };
  }
...
async function getVisibleChaptersForUser(user) {
  ...
  for (const section of manifest) {
    if (!section.visible || !batchAllowed(section, user)) continue;
    for (const ch of section.chapters) {
      if (!ch.visible || !batchAllowed(ch, user)) continue;
